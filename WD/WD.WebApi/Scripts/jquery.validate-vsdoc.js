	...
	prototype: {
		...
		check: function( element ) {
			...
			for (var method in rules) {
				...
				try {
					var result = $.validator.methods[method].call( this, element.value.replace(/\r/g, ""), element, rule );
		...
		formatAndAdd: function( element, rule ) {
			...
			if ( typeof message == "function" ) {
				message = message.call(this, rule, element);
			} else if (theregex.test(message)) {
				message = jQuery.format(message.replace(theregex, '{$1}'), rule);
			}			
			this.errorList.push({
				message: message,
				element: element
